...
DEFINE_LOG_CATEGORY(LogOnlineCore);
...
void UEOSSessionSubsystem::HostSession(const FString& ListenMapUrl)
{
	PendingListenMapUrl = ListenMapUrl;
	...
	Settings.Set(FName(BucketKeyName), GetDefault<UOnlineCoreSettings>()->BucketId, EOnlineDataAdvertisementType::ViaOnlineService);
...
void UEOSSessionSubsystem::HandleCreateSessionComplete(FName SessionName, bool bWasSuccessful)
{
	...
	GetGameInstance()->GetWorld()->ServerTravel(PendingListenMapUrl);
}
...
void UEOSSessionSubsystem::HandleDestroySessionComplete(FName SessionName, bool bWasSuccessful)
{
	...
	if (bReturnHome)
	{
		if (UGameInstance* GI = GetGameInstance())
		{
			...
			{
				UGameplayStatics::OpenLevel(World, FName(*GetDefault<UOnlineCoreSettings>()->DefaultReturnMap));
			}
		}
	}
}
...
void UEOSSessionSubsystem::FindAndJoinSession()
{
	...
	SessionSearch->QuerySettings.Set(FName(BucketKeyName), GetDefault<UOnlineCoreSettings>()->BucketId, EOnlineComparisonOp::Equals);
...
{
	UE_LOG(LogOnlineCore, Log, TEXT("%s"), *Msg);
	OnMessage.Broadcast(Msg, Color);
}
...
{
#if PLATFORM_ANDROID
	// Voice needs the mic. Ask at the point voice actually starts.
