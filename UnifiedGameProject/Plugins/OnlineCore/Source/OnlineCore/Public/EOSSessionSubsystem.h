...
ONLINECORE_API DECLARE_LOG_CATEGORY_EXTERN(LogOnlineCore, Log, All);

DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FOnlineCoreMessage, const FString&, Message, FColor, Color);
...
UCLASS()
class ONLINECORE_API UEOSSessionSubsystem : public UGameInstanceSubsystem
{
	GENERATED_BODY()

public:
	/** Every status message. The game decides whether to draw it. */
	UPROPERTY(BlueprintAssignable, Category = "EOS")
	FOnlineCoreMessage OnMessage;
	...
	UFUNCTION(BlueprintCallable, Category = "EOS")
	void HostSession(const FString& ListenMapUrl);
...
private:
	...
	/** Held from HostSession() until HandleCreateSessionComplete() travels. */
	FString PendingListenMapUrl;
