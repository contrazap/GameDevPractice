# P3 — Genre Systems Library (post-release menu)

Goal: the mechanics library for later episodes/DLC — each feature is a self-contained system practiced in the umbrella project, in its own test level or bolted onto the P1 slice where it fits. No dates. Entries are brief; the generator expands them on demand.
...
Items as DataAssets, slot/grid inventory, replicated pickup/drop/transfer, equipment slots feeding the P1.3 weapon system, container UI (lockers upgrade). **Maps to**: PUBG, Zomboid. **Prereqs**: P1.1, P2.4. **Est**: 3.
